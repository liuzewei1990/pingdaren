...
import lrz from "lrz";
import { upload } from '@src/apis/upload.js'
...
export default {
    install(Vue) {
        ...
        Vue.prototype.initilize = function (opt) {
            ...
            // debugger;
            //初始化裁剪对象 api文档 https://blog.csdn.net/qq727013465/article/details/51823231
        ...
        //创建一些必要的DOM，用于图片裁剪
        Vue.prototype.createElement = function (originUrl) {
            //初始化图片为空对象
            this.preview = null;
            ...
            str += '<div class="crop_loading"><div class="crop_content"><img src=""><div class="crop_text">图片上传中</div></div></div>';
        ...
        //图片大小验证
        Vue.prototype.verificationPicFile = function (file) {
            var fileSize = 0;
            var fileMaxSize = 4096;//4M
        ...
        Vue.prototype.initClip = function (e, opt) {
            let self = this;
            this.verificationPicFile(e.target);
            ...
            this.createElement(this.originUrl);
            setTimeout(() => {
                this.initilize(opt);
            }, 0);
            return;

            lrz(this.picValue, { width: 1000 })
                .then(res => {
                    // this.originUrl = res.base64;
                    // alert(this.originUrl)
                    //每次替换图片要重新得到新的url
                    // if (this.cropper) {
        ...
        Vue.prototype.postImg = function (imageData) {
            ...
            upload({
                ...
                self.resultObj && (self.resultObj.src = imageData);
                if (typeof this.options.callback === "function" && imageData) this.options.callback(data);
                //裁剪完后摧毁对象
                self.destoried();
            }, (err) => {
                document.querySelector('.crop_loading').style.display = 'none';
